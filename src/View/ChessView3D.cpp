...
void ChessView3D::draw(ChessGame& game, ViewContext& ctx)
{
    ...
    // 1. Calculate logical and physical sizes for High-DPI (Retina) support
    ImVec2 logicalSize = ImGui::GetContentRegionAvail();
    if (logicalSize.x <= 0.0f)
        logicalSize.x = 600.0f;
    if (logicalSize.y <= 0.0f)
        logicalSize.y = 600.0f;

    ImVec2 fbScale      = ImGui::GetIO().DisplayFramebufferScale;
    ImVec2 physicalSize = ImVec2(logicalSize.x * fbScale.x, logicalSize.y * fbScale.y);

    m_framebuffer->resizeIfNeeded(static_cast<int>(physicalSize.x), static_cast<int>(physicalSize.y));

    // 2. Hover Calculation (Uses logical coordinates)
    const ImVec2            pos = ImGui::GetCursorScreenPos();
    std::optional<Position> hoveredPos;

    if (ImGui::IsMouseHoveringRect(pos, ImVec2(pos.x + logicalSize.x, pos.y + logicalSize.y)))
    {
        ImVec2    mousePos(ImGui::GetMousePos());
        glm::vec2 localMousePos(mousePos.x - pos.x, mousePos.y - pos.y);
        glm::vec2 windowSizeLogical(logicalSize.x, logicalSize.y);

        hoveredPos = m_mousePicker->getBoardPosition(localMousePos, windowSizeLogical, m_activeCamera->getViewMatrix(), m_activeCamera->getProjectionMatrix(logicalSize.x, logicalSize.y));
    }

    // 3. Render
    renderPipeline(game, ctx, logicalSize, hoveredPos);

    // 4. Interaction
    ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(m_framebuffer->getTextureId())), logicalSize, ImVec2(0, 1), ImVec2(1, 0));

    ImGui::SetCursorScreenPos(pos);
    ImGui::InvisibleButton("##3DViewBtn", logicalSize);
...
void ChessView3D::renderPipeline(const ChessGame& game, const ViewContext& ctx, const ImVec2& size, const std::optional<Position>& hoveredPos)
{
    ...
    m_sceneRenderer->drawScene(game, ctx, m_activeCamera->getViewMatrix(), m_activeCamera->getProjectionMatrix(size.x, size.y), hoveredPos, m_visualState);

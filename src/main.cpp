    ...
    quick_imgui::loop(
        ...
        {.init = [&]() {
         ...
         .loop = [&]() {
            ...
            ImGui::Text("FEN (Copier pour analyser):");
            std::string fen = game.getFEN();
            ImGui::InputText("##fen", (char*)fen.c_str(), fen.size(), ImGuiInputTextFlags_ReadOnly);

            ImGui::Separator();
            ImGui::Text("Historique :");
            ImGui::BeginChild("ScrollingRegion", ImVec2(0, -1), false, ImGuiWindowFlags_HorizontalScrollbar);
            ...
            for (int i = 0; i < history.size(); ++i)
            {
                if (i % 2 == 0)
                {
                    ImGui::Text("%d. %s", (i / 2) + 1, history[i].c_str());
                }
                else
                {
                    ImGui::SameLine();
